...
import logging
from .conv import create_linearblock, create_convblock1d
from .PCM.PCM import PointMambaEncoder, PointMambaDecoder
import numpy as np
...
class BaseSeg(nn.Module):
    def __init__(self,
                 ...
                 **kwargs):
        super().__init__()
        self.encoder = PointMambaEncoder(**encoder_args)
        if decoder_args is not None:
            ...
            self.decoder = PointMambaDecoder(**decoder_args_merged_with_encoder)
        else:
            self.decoder = None
